...
class CpmTest(unittest.TestCase):
    ...
    def test_valid_read_cpm(self):
        golden_path = os.path.normpath(os.path.join(os.getcwd(), "../test_fixtures/golden/cpm_breast.csv"))
        golden = pd.read_csv(golden_path, index_col=0)

        data_path = os.path.normpath(os.path.join(os.getcwd(), "../test_fixtures/data_info_breast.csv"))
        ...
        pd.testing.assert_frame_equal(golden, res, check_dtype=False, check_like=True, check_exact=False, atol=0.1,
                                      rtol=0.1)
        # return pd.read_csv("test_fixtures/golden/cpm_breast.csv", index_col=0)
