...
class BatchEffectRemovalTest(unittest.TestCase):
    def setUp(self):
        ...
        self.golden_batch = csv_to_dataframe(
            path_components=["test_fixtures", "golden", "batch_matrix_sva_breast.csv"], index_col=0, header=0)

    def test_rna_seq_qa(self):
        qa_matrix = csv_to_dataframe(
            path_components=["test_fixtures", "golden", "qa_matrix_breast.csv"], header=0, index_col=0)
        quality_labels = csv_to_dataframe(
            path_components=["test_fixtures", "golden", "qa_labels_breast.csv"]).iloc[:, 0]
